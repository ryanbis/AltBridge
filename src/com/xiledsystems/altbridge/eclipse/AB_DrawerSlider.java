...
public class AB_DrawerSlider extends ViewGroup {
    ...
    private static final float          MAXIMUM_TAP_VELOCITY            = 300.0f;
     ...
     */
    public AB_DrawerSlider( Context context, AttributeSet attrs )
    {
     ...
     */
    public AB_DrawerSlider( Context context, AttributeSet attrs, int defStyle )
    {
        super( context, attrs, defStyle );
        TypedArray a = context.obtainStyledAttributes( attrs, R.styleable.DrawerSlider, defStyle, 0 );
        
        String o = a.getString(R.styleable.DrawerSlider_direction);     
        int orientation = getOrientInt(o);
        mVertical = ( orientation == ORIENTATION_BTT || orientation == ORIENTATION_TTB );
        mBottomOffset = (int)a.getDimension( R.styleable.DrawerSlider_bottomOffset, 0.0f );
        mTopOffset = (int)a.getDimension( R.styleable.DrawerSlider_topOffset, 0.0f );
        mAllowSingleTap = a.getBoolean( R.styleable.DrawerSlider_allowSingleTap, true );
        mAnimateOnClick = a.getBoolean( R.styleable.DrawerSlider_animateOnClick, true );
        mInvert = ( orientation == ORIENTATION_TTB || orientation == ORIENTATION_LTR );
        
        int handleId = a.getResourceId( R.styleable.DrawerSlider_handle, 0 );
        if ( handleId == 0 ) { throw new IllegalArgumentException( "The handle attribute is required and must refer "
                + "to a valid child." ); }
        
        int contentId = a.getResourceId( R.styleable.DrawerSlider_content, 0 );
    ...
    public void setAnimateOnClick(boolean animate) {
      mAnimateOnClick = animate;
    }
    
    public boolean getAnimateOnClick() {
      return mAnimateOnClick;
    }
    
    private int getOrientInt(String o) {
        if (o.equalsIgnoreCase("toptobottom")) {
            return 3;
        } else if (o.equalsIgnoreCase("lefttoright")) {
            return 2;
        } else if (o.equalsIgnoreCase("righttoleft")) {
            return 0;
        }
        return 1;
    }
    ...
    public void hideHandle() {      
        if (!(mHandle.getLayoutParams().height == 0) || !(mHandle.getLayoutParams().width == 0)) {
            mHandle.getLayoutParams().height = 0;
            mHandle.getLayoutParams().width = 0;
            mHandle.requestLayout();
        }
    }
    
    public void showHandle(int width, int height) {
        if ((mHandle.getLayoutParams().height == 0) || (mHandle.getLayoutParams().width == 0)) {
            
            mHandle.getLayoutParams().height = height;
            mHandle.getLayoutParams().width = width;
            mHandle.invalidate();
            mHandle.requestLayout();
        }
    }
    ...
    /**
     * 
     * Manually set the content through code.
     * 
     * @param content
     */
    ...
    /*public void setContentAndSwitch(SketchScreen context, int resourceId) {
        LayoutInflater layout = context.getLayoutInflater();
        removeView(mContent);
        mContent = null;
        View v = layout.inflate(resourceId, null);
        //mContent = v;     
        mContentId = v.getId();
        //onFinishInflate();
        mContent = findViewById(mContentId);
    ...
    public void SetBottomToTopDirection() {     
        if( mInvertStart ) {
            mMaximumAcceleration = -mMaximumAcceleration;
            mMaximumMajorVelocity = -mMaximumMajorVelocity;
            mMaximumMinorVelocity = -mMaximumMinorVelocity;
