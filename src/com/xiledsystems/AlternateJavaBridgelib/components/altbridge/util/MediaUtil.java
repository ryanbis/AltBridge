...
//import com.xiledsystems.AlternateJavaBridgelib.components.altbridge.ReplForm;
...
return openMedia(formservice, mediaPath, determineMediaSource(formservice, mediaPath));
  ...
  private static File copyMediaToTempFile(Form form, String mediaPath, MediaSource mediaSource)
    ...
    try {
      ...
      // TODO(user) - figure out how much space is left on the SD card and log that
      // information.
      throw e;
  ...
  public static File copyMediaToTempFile(FormService formservice, String mediaPath)
  throws IOException {
...
private static File copyMediaToTempFile(FormService formService, String mediaPath, MediaSource mediaSource)
  throws IOException {
...
try {
  ...
  private static File cacheMediaTempFile(FormService formservice, String mediaPath, MediaSource mediaSource)
  throws IOException {
File tempFile = tempFileMap.get(mediaPath);
// If the map didn't contain an entry for mediaPath, or if the temp file no longer exists,
// copy the file to a new temp file.
...
return tempFile;
  ...
  public static Drawable getDrawable(Form form, String mediaPath) throws IOException {
    ...
    if (mediaSource == MediaSource.ASSET) {
    	...
    	int temp = form.getResources().getIdentifier(mediaPath, "drawable", form.getPackageName());
    	try {
    		return form.getResources().getDrawable(temp);
  ...
  public static int loadSoundPool(SoundPool soundPool, Form form, String mediaPath)
    ...
    switch (mediaSource) {
      case ASSET:
        ...
        int temp = form.getResources().getIdentifier(mediaPath, "raw", form.getPackageName());
        return soundPool.load(form, temp, 1);
  ...
  public static int loadSoundPool(SoundPool soundPool, FormService formService, String mediaPath)
  throws IOException {
MediaSource mediaSource = determineMediaSource(formService, mediaPath);
switch (mediaSource) {
  case ASSET:
...
public static void loadMediaPlayer(MediaPlayer mediaPlayer, Form form, String mediaPath)
  ...
  @SuppressWarnings("static-access")
public static void loadMediaPlayer(MediaPlayer mediaPlayer, FormService formservice, String mediaPath)
  throws IOException {
MediaSource mediaSource = determineMediaSource(formservice, mediaPath);
switch (mediaSource) {
  case ASSET:
