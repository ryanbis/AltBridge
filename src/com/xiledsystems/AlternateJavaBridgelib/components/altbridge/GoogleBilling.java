...
/**
 * Convenience class for using Google Billing for in-app purchases. You need to add <uses-permission
 * android:name="com.android.vending.BILLING" /> to your applications AndroidManifest file for this to work.
 * 
 * You will also need to create the package com.android.vending.billing in your project, and download the Google Market
 * Billing from the SDK manager, and place the IInAppBillingService.aidl file into the newly created package.
 * 
 * @author Ryan Bis
 * 
 */
public class GoogleBilling extends AndroidNonvisibleComponent implements ActivityResultListener, OnDestroyListener {
	...
	/**
	 * Launch the Google Billing purchase process. This throws the PURCHASE_FINISHED event if the purchase is finished
	 * successfully. Make sure to check the purchase with purchase.getPurchaseState, as the three states are PURCHASED,
	 * CANCELLED, and REFUNDED.
	 * 
	 * @param item
	 *            - The sku of the item to purchase. This should match what you have in the developer's console
	 * @param type
	 *            - The type of purchase (ITEM_TYPE_INAPP, or ITEM_TYPE_SUBS for subscriptions)
	 */
	public void PurchaseItem(String item, String type) {
		if (helper != null) {
			tmpSku = item;
			helper.launchPurchaseFlow(container.$context(), item, requestCode, purchaseFinsihedListener, payload);
		} else {
	...
	private class SetupFinishedListener implements OnIabSetupFinishedListener {
		...
		public void onIabSetupFinished(IabResult result) {
			if (!result.isSuccess()) {
				EventDispatcher.dispatchEvent(GoogleBilling.this, Events.BILLING_ERROR, "Problem setting up in-app " + "billing. " + "Error: "
						+ result);
			} else {
				helper.queryInventoryAsync(gotInventoryListener);
			}
	...
	private class InventoryListener implements IabHelper.QueryInventoryFinishedListener {
		...
		public void onQueryInventoryFinished(IabResult result, Inventory inv) {
			if (result.isFailure()) {
				EventDispatcher.dispatchEvent(GoogleBilling.this, Events.BILLING_ERROR, "Failure querying inventory. Error: " + result.toString());
			} else {
		...
		public void onIabPurchaseFinished(IabResult result, Purchase info) {
			if (result.isFailure()) {
				...
				if (result.toString().contains(NULL_DATA_ERROR) && tmpSku != null) {
					...
					EventDispatcher.dispatchEvent(GoogleBilling.this, Events.BILLING_ERROR, "Error purchasing. Error: " + result.toString());
				}
