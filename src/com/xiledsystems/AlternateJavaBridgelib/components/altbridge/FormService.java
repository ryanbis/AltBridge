...
/**
 * This is the service class for the AltBridge. Writing for a service is similar to a Form in that
 * you still have a $define method, and the event system. However, a service cannot directly
 * touch the UI, so no visible components may be used in a service. Communication can be
 * achieved between a Form and FormService using the SendMessageToForm method (you must make
 * sure the service is bound to the Form first).
 * If you need to use a thread, then use a ThreadTimer.
 * 
 * To make sure the service doesn't get arbitrarily killed by the android system, use
 * setStickyVal(START_STICKY). When using this, make sure you leave some way for the app
 * user to shut it off (using the stopService method). They shouldn't have to navigate
 * through their system settings just to turn your service off, if they wanted to.
 * 
...
public class FormService extends Service implements Component, SvcComponentContainer, HandlesEventDispatching {
	 ...
	 * 
	 * @param message The message you'd like to send. Can be anything in string format.
	 */
