...
public class SlideDrawer extends ViewGroup {
	...
	public SlideDrawer( Context context, int handleId, int contentId) {
		super(context);
		
		int orientation = context.getResources().getIdentifier("DrawerSlider_direction", "styleable", context.getPackageName());
		
		mVertical = ( orientation == ORIENTATION_BTT || orientation == ORIENTATION_TTB );
		mBottomOffset = context.getResources().getIdentifier("DrawerSlider_bottomOffset", "styleable", context.getPackageName());
		mTopOffset = context.getResources().getIdentifier("DrawerSlider_topOffset", "styleable", context.getPackageName());
	...
	public SlideDrawer( Context context, AttributeSet attrs, int defStyle )
	{
		...
		for (int i = 0; i < count; i++) {
			...
			if (attrs.getAttributeName(i).equalsIgnoreCase("direction")) {
				...
				orientation = Integer.parseInt(id);
			}
			if (attrs.getAttributeName(i).equalsIgnoreCase("bottomOffset")) {
				String id = attrs.getAttributeValue(i).replace("@", "");				
				mBottomOffset = Integer.parseInt(id);
			}
			if (attrs.getAttributeName(i).equalsIgnoreCase("topOffset")) {
				String id = attrs.getAttributeValue(i).replace("@", "");				
				mTopOffset = Integer.parseInt(id);
			}
			if (attrs.getAttributeName(i).equalsIgnoreCase("allowSingleTap")) {
				String id = attrs.getAttributeValue(i).replace("@", "");				
				mAllowSingleTap = Boolean.parseBoolean(id);
			}
			if (attrs.getAttributeName(i).equalsIgnoreCase("animateOnClick")) {
				String id = attrs.getAttributeValue(i).replace("@", "");				
				mAnimateOnClick = Boolean.parseBoolean(id);
			}
		}
	...
	public void setDrawerDirection(int direction) {
		boolean temp = mInvert;
		switch (direction) {		
			case Form.DRAWER_BOTTOM_TO_TOP:
				mVertical = true;
				mInvert = false;
				break;			
			case Form.DRAWER_RIGHT_TO_LEFT:
				mVertical = false;
				mInvert = false;
				break;			
			case Form.DRAWER_LEFT_TO_RIGHT:
				mVertical = false;
				mInvert = true;
				break;			
			case Form.DRAWER_TOP_TO_BOTTOM:
				mVertical = true;
				mInvert = true;
				break;
		}
	...
	protected void onFinishInflate()
	{
		//if (!isInEditMode()) {
			...
			if ( mHandle == null ) { throw new IllegalArgumentException( "The handle attribute must refer to an" + " existing child." ); }
			mHandle.setOnClickListener( new DrawerToggler() );
		
			mContent = findViewById( mContentId );
			if ( mContent == null ) { throw new IllegalArgumentException( "The content attribute must refer to an"
				+ " existing child." ); }
