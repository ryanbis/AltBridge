...
public final class AnimCanvas extends AndroidViewComponent implements DrawingCanvas, ComponentContainer, OnResumeListener, OnStopListener, 
	 ...
	 * or {@link Canvas_backup#Touched(float, float, boolean)}, respectively.
	 * The additional final argument indicates whether it was preceded by one or
			...
			switch (event.getAction()) {
				...
				case MotionEvent.ACTION_MOVE:
					...
					if (startX == UNSET || startY == UNSET || lastX == UNSET || lastY == UNSET) {
						Log.w("Canvas", "In Canvas.MotionEventParser.parse(), " + "an ACTION_MOVE was passed without a preceding ACTION_DOWN: "
								+ event);
					}
					...
					boolean handled = throwSpriteDraggedEvents(draggedSprites, startX, startY, lastX, lastY, x, y);
				...
				case MotionEvent.ACTION_UP:
					...
					if (!drag) {
						// It's a tap
						handled = processSpriteUpEvent(sprites, draggedSprites, x, y);
	...
	private void stoppedDragging() {
		if (eventListener != null) {
			...
			EventDispatcher.dispatchEvent(this, Events.DONE_DRAGGING);
		}
	}
	...
	private final class CanvasView extends SurfaceView implements SurfaceHolder.Callback {
		...
		public CanvasView(Context context) {
			super(context);
			viewBitmap = Bitmap.createBitmap(ComponentConstants.CANVAS_PREFERRED_WIDTH, ComponentConstants.CANVAS_PREFERRED_HEIGHT,
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			// sequence.
			container.getRegistrar().dontGrabTouchEventsForComponent();
	...
	/**
	 * This sets the cycle rate at which the canvas redraws. The default is 50
	 * ms. (This equates to 20fps). Adjust this to suit your needs. Remember
	 * that there is a point of dimishing returns here. For instance, if you set
	 * this to 1 (as in 1 millisecond), it's highly unlikely that the canvas
	 * will actually redraw every 1 ms, as it will most likely take at least
	 * that long to draw. Most people can't tell the difference above 40 or 50
	 * fps (50 fps would mean you need to set the cyclerate to 25).
	 * 
	 * @param cycle
	 *            The cycle at which the canvas redraws itself in ms.
	 */
	...
	/**
	 * 
	 * @return The cyclerate at which the canvas redraws itself in ms. (Default
	 *         50)
	 */
	...
	public AnimCanvas(ComponentContainer container, int resourceId) {
		...
		// params = (RelativeLayout.LayoutParams) rl.getLayoutParams();
		LinearLayout layout1 = new LinearLayout(container, LAYOUT_ORIENTATION_VERTICAL);
		rl.addView(layout1.getLayoutManager(), new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.MATCH_PARENT,
		...
		if (rl.getBackground() != null) {
			...
			if (backgroundDrawable instanceof ColorDrawable) {
				...
				// Will have to think of something here. This is a problem if
				// the dev is
				// building their app using an API older than 11 (HoneyComb
				// 3.0). Older apis
				// didn't have the getColor() method. This method works on older
				// phones as long as
				// the app is built from a later API (SDKLevel). For now, will
				// throw an error in
				// logcat to notify the dev.
				...
				try {
					...
					Log.e("AnimCanvas", "Phone's SDKLevel is below 11. You will have to set the color of the canvas' background through code, "
							+ "if you want this to work on older OS's.");
				}
	...
	private static Bitmap createBitmap(Bitmap bitmap, int width, int height) {
		...
		if (destBitmap.getWidth() != width || destBitmap.getHeight() != height) {
	...
	/**
	 * This sets the frames per second of the background animation (when using
	 * setAnimListandFPS).
	 * 
	...
	public void startAnimation() {
		// We have to implement our own animation handler here, as setting
		// the SurfaceView's background will place the background on top
		// of everything, covering any sprites.
		if (!animRunning) {
			animRunning = true;
			running = true;
			animThread = new Thread(new Runnable() {
				...
				public void run() {
					...
					while (running) {
						...
						// seperate thread
						// cannot touch
						// any views. While this may seem to defeat the purpose,
						// the
						// timing for the animation
						// is handled in a seperate thread, rather than the UI
						// thread
						// (which has other
						// stuff to process).

						getRegistrar().post(new Runnable() {
							@Override
							public void run() {
						...
						// If there is time to spare between frames, go to sleep
						// for this
						// long
						...
						// If the current frame is higher than the framecount,
						// go back to
						// the first frame
						// If the animation is not set to loop, then it stops
						// here.
						if (curframe >= animBackground.getNumberOfFrames()) {
							curframe = 0;
							if (!loop) {
								running = false;
								animRunning = false;
						...
						// This increases the currentframe which resides outside
						// of the
						// thread.
						curFrame(curframe);
					}
	...
	private void setAnimBackground() {
		if (picList.size() > 0) {
			...
			if (animBackground != null) {
				if (Width() == 0 || Height() == 0) {
					viewBitmap = ((BitmapDrawable) animBackground.getFrame(0)).getBitmap();
				} else {
					viewBitmap = createAndSizeAnimBitmap(animBackground, 0, Width(), Height());
				}
				clearViewCanvas();
			}
		}
	...
	public void setFrame(int frame) {
		...
		if (animBackground != null) {
			if (frame < animBackground.getNumberOfFrames()) {
				if (Width() == 0 || Height() == 0) {
					viewBitmap = ((BitmapDrawable) animBackground.getFrame(frame)).getBitmap();
				} else {
					viewBitmap = createAndSizeAnimBitmap(animBackground, frame, Width(), Height());
				}
				clearViewCanvas();
			}
		}
	...
	private static Bitmap createAndSizeAnimBitmap(AnimationDrawable draw, int frame, int width, int height) {
		return Bitmap.createScaledBitmap(((BitmapDrawable) draw.getFrame(frame)).getBitmap(), width, height, true);
	}
	...
	/**
	 * Alternate method to set the background image. Used with SpriteSheetHelper
	 * when managing images in a sprite sheet.
	 * 
	 * @param drawable
	 */
	...
	public void DrawRectangle(int x1, int y1, int x2, int y2) {
		view.canvas.drawRect(x1, y1, x2, y2, paint);
		view.invalidate();
	}
	 ...
	 * 
	 */
	public void DrawColorGradient(int x1, int y1, int x2, int y2, int hue) {
		Paint pt = new Paint();
		...
		pt.setShader(shader);
		view.canvas.drawRect(x1, y1, x2, y2, pt);
		view.invalidate();
	}
		...
		try {
			...
			try {
				if (SdkLevel.getLevel() >= SdkLevel.LEVEL_DONUT) {
					...
					// and
					// in the getDrawingCache call below because when it is true
					// and
					// we're running in compatibility mode, attempting to set
					// the
					// canvas size to a width X height that exceeds some
					// threshold
					// (somewhere
					// around 75,600 pixels) causes the getDrawingCache call to
					// return
					// null.
					// I couldn't figure out why this happens, but setting
					// autoscale
				...
				try {
					...
					if (bitmap != null) {
						success = bitmap.compress(format, 100, // quality:
																// ignored for
																// png
								fos);
					}
	...
	private static BoundingBox newBoundingBox(float x, float y, float width, float height) {
		return new BoundingBox(Math.max(0, (int) x - MotionEventParser.HALF_FINGER_HEIGHT),
				Math.max(0, (int) y - MotionEventParser.HALF_FINGER_WIDTH), Math.min(width - 1, (int) x + 
						MotionEventParser.HALF_FINGER_WIDTH), Math.min(height - 1, (int) y + MotionEventParser.HALF_FINGER_HEIGHT));
	}

	private static int[] processCanvasMovement(int[] rtn, int xDifference, int yDifference, int oWidth, int oHeight, int srcWidth,
			int srcHeight) {
		rtn[0] = xDifference;
		rtn[1] = yDifference;
	...
	public void onDestroy() {
		super.onDestroy();
		if (oversizeBitmap != null) {
			oversizeBitmap.recycle();
		}
		if (destBitmap != null) {
			destBitmap.recycle();
		}
	...
	public void canvasInitialized() {
		...
		new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {			
			@Override
			public void run() {
				EventDispatcher.dispatchEvent(AnimCanvas.this, Events.CANVAS_INIT);
			}
		}, 25);	
	}
