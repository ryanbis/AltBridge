...
public class SoapClient extends AndroidNonvisibleComponent {
	 ...
	 * 
	 * NOTE: SoapHelper MUST be instantiated before SoapClient or you will
	 * receive a null pointer exception
	 * 
	 * @param container
	 *            Always use this
	 * @param helper
	 *            The SoapHelper you used to create a request.
	 */
				...
				try {
					...
					if (connection.getResponseCode() <= 400) {
						if (!connection.getResponseMessage().trim().equals("OK")) {
							Log.e("SoapClient",
									"Couldn't complete request. Denied by host. Response Message From Server: " + connection.getResponseMessage());
							return;
						} else {
	...
	/**
	 * 
	 * This will return the response the SoapClient received from the request.
	 * This should be run after the SoapResponse event fires.
	 * 
	 ...
	 * 
	 * @param stripHtml
	 *            - If set to true, this will strip out HTML codes (like &amp;)
	 *            This simply calls Html.fromHtml(response).toString()
	 * 
	 * @return the response from the soap server
	 */
	public String Response(boolean stripHtml) {
		if (stripHtml) {
			return Html.fromHtml(soapResponse).toString();
		} else {
			return soapResponse;
		}
	}
	...
	/**
	 * This method is called internally when it receives a response from the
	 * Soap server.
	 * 
	 * @param response
	 */
	public void SendResponseEvent(final String response) {
		...
		post(new Runnable() {
			@Override
			public void run() {
				EventDispatcher.dispatchEvent(client, Events.SOAP_RESPONSE);
			}
		});
	}
